    ...
    public IEnumerable<V1EnvVar> GenerateEnvVars(PatchingContext context)
    {
        if (_injectorOptions.EnableEarlyChaining)
        {
            yield return new V1EnvVar("LD_PRELOAD",
                $"{context.AgentMountPath}/contrast/runtimes/linux/native/ContrastChainLoader.so");
        }
        else
        {
            yield return new V1EnvVar("CORECLR_PROFILER", "{8B2CE134-0948-48CA-A4B2-80DDAD9F5791}");
            yield return new V1EnvVar("CORECLR_PROFILER_PATH", $"{context.AgentMountPath}/contrast/runtimes/linux-x64/native/ContrastProfiler.so");
            yield return new V1EnvVar("CORECLR_PROFILER_PATH_ARM64", $"{context.AgentMountPath}/contrast/runtimes/linux-arm64/native/ContrastProfiler.so");
